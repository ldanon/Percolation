...
plotgraph<-function(degseq,p){
  g <- sample_degseq(bestcase)
...
#  colbar <- rainbow(max(comps)+1)
  ...
  colbar = rep("white",max(comps)+1)
  colbar[2] = "red"
  
  removenumberlow = round((1-p-0.1)*nl) # number of links to remove. 
  removenumber = round((1-p)*nl)
  removenumberhigh = round((1-p+0.1)*nl)
  
  sub_g_low<-delete_edges(g,sample(E(g),removenumberlow))
  LC = which.max(components(sub_g_low)$csize)
  V(sub_g_low)$color <- "white"
  V(sub_g_low)$color[components(sub_g_low)$membership==LC] <- "red"
  
  sub_g<-delete_edges(sub_g_low,sample(E(sub_g_low),removenumber - removenumberlow))
  LC = which.max(components(sub_g)$csize)
  V(sub_g)$color <- "white"
  V(sub_g)$color[components(sub_g)$membership==LC] <- "red"
  

  sub_g_high<-delete_edges(sub_g,sample(E(sub_g),removenumberhigh-removenumber))
  LC = which.max(components(sub_g_high)$csize)
  V(sub_g_high)$color <- "white"
  V(sub_g_high)$color[components(sub_g_high)$membership==LC] <- "red"

  par(mfrow = c(1,3),mar = c(1,1,1,1))
  plot(sub_g_high, layout=coords, vertex.size=6, vertex.label=NA)
      mtext('Best Case',side=1,line=-7)
  #     mtext(expression(p<p[c]),side=1,line=-7)
      plot(sub_g, layout=coords, vertex.size=6, vertex.label=NA)
      mtext('Reasonable Case',side=1,line=-7)
      #      mtext(expression(p==p[c]),side=1,line=-7)
      plot(sub_g_low, layout=coords, vertex.size=6, vertex.label=NA)
#      mtext(expression(p>p[c]),side=1,line=-7)
      mtext('Worst Case',side=1,line=-7)
  }
